"use strict";
/* eslint-disable no-shadow */ const HEADERS = {
    name: "Name",
    position: "Position",
    office: "Office",
    age: "Age",
...
function handleSortClick(header) {
    const index = [
        ...head.children
    ].indexOf(header.target);
    currentSortState.order = currentSortState.order > 0 ? -1 : 1;
    const order = currentSortState.sortedBy === header.target.innerText ? currentSortState.order : 1;
    const sortedList = [
        ...list
    ].sort((a, b)=>{
...
function handleCellEdit(e) {
    ...
    const newInputType = Number.isNaN(toNum(editedCell.innerText)) ? "text" : "number";
